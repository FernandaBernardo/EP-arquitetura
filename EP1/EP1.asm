...
main:
	...
	add $a3, $v0, $zero # pega o sort escolhido pelo usuario
...
get_file_name:
# Retorna em $v0 o endereco para o nome do arquivo. Usa do dialog para input
	...
	la $a1, file_name 	# carrega o local onde sera armazenado o input do usuario
	lw $a2, min_size 	# carrega o numero de bytes que serao lidos
...
# tirar o caractere que determina o final da string que nao serve quando for usar para abrir o arquivo	
   	li  $t0, 0       	
   	lw  $t1, min_size	      	# inicializando o final do loop
    
remove_line_feed_loop:
	beq $t0, $t1, back_stack	# verifica se o loop chegou ao final
    lb  $t3, file_name($t0) 		# pega o byte de determinada posicao do nome do arquivo
    bne $t3, 10, cont_remove_line_feed_loop # verifica se esse byte contem o caracter 10 (linefeed)
...
# Abre pra leitura o arquivo de entrada especificado em $a0
	...
	# e esperado que $a0 contenha o endereco para o nome do arquivo
	li   $a1, 0     	# parametro para abrir para leitura (0: leitura, 1: escrita)
	syscall         	# syscall: abre arquivo e armazena em $v0 o descritor de arquivo
	
	slt  $t0, $v0, $zero	# testa se eh menor que zero. se sim, houve erro.
	beq  $t0, 1, exit_error
	...
	addi $sp, $sp, -16	# aloca espaco na pilha para salvar variaveis atuais
	...
	# t0 = descritor do arquivo, t1 = endereco base do array, t2 = contador
	
	add $a0, $zero, $t2	# parametro esperado para funcao, inicialmente 1, no caso
	jal allocate_memory	# armazena em $v0 o endereco de min_size bytes recem alocados
	add $t1, $v0, $zero	# salva o endereco base do array de bytes alocado
	add $s0, $v0, $zero	
		
	lw $ra, 12($sp)		# restaura endereco de retorno - comeca restauracao
	lw $fp,  8($sp)		# restaura frame pointer
	lw $t2,  4($sp)		# restaura contador
	lw $t0,  0($sp)		# restaura descritor do arquivo
	add $sp, $sp, 16	# desaloca espaco na pilha - fim da restauracao

read:	
	li   $v0, 14        	# carrega chamada de sistema para ler do arquivo
	add  $a0, $t0, $zero 	# guarda em $a0 o descritor do arquivo
	add  $a1, $t1, $zero   	# carrega endereco inicial de onde serao guardados os bytes lidos
	lw   $a2, min_size     	# carrega limite maximo de leitura por iteracao
	syscall             	# syscall: le no max. $a2 bytes do arquivo e armazena em $v0 a qtde de bytes lidas de fato

	bltz $v0, exit_error

	slt  $t3, $v0, $a2	# testa se o numero de bytes lido e menor que o limite por iteracao 
	beq  $t3, 1, close_file # se leu menos, fecha o arquivo e salva dados lidos
				# senao, aloca mais memoria para o array e continua leitura
	
	# t0 = descritor do arquivo, t1 = endereco base do array, t2 = contador	
	addi $t2, $t2, 1	# incrementa contador (fator de mult. de numero de bytes a serem alocados)
	
	addi $sp, $sp, -24	# aloca espaco na pilha para salvar estado atual
	sw   $ra, 20($sp)	# salva retorno 
	sw   $fp, 16($sp)	# salva frame pointer atual
	sw   $t0, 12($sp)	# salva descritor do arquivo
	sw   $t1, 8($sp)	# salva endereco base do array
	sw   $t2, 4($sp)	# salva contador atual
	...
	# t0 = descritor do arquivo, t1 = endereco base do array, t2 = contador
	add $a0, $zero, $t2
	jal allocate_memory	# retorna em $v0 o endereco de $a0 * min_size bytes recem alocados
	add $s0, $v0, $zero	# armazena em $s0 o endereco base do array com a copia
	
	add  $a1, $zero, $v0	# (parametro) endereco base do array recem alocado que armazenara a copia - destino
	...
	mul  $a2, $a2, $t0	# (parametro) numero de elementos a serem copiados da origem = ( contador - 1 ) * min_size
	lw   $a0, 8($sp)	# (parametro) endereco base do array a ser copiado - origem
	add  $t3, $t2, -2
	mul  $t3, $t3, $t0
	sub  $a0, $a0, $t3
	...
	lw   $t1, 8($sp)	# restaura endereco base do array
	lw   $t2, 4($sp)	# restaura contador
	lw   $v0, 0($sp)	# restaura retorno atual
	addi $sp, $sp, 24	# desaloca espaco na pilha para salvar estado atual	
	...
	addi $t4, $t2, -1
	mul  $t4, $t4, $t3
	add  $t1, $s0, $t4	# armazena em t1 o endereco base do array da copia, deslocado das copias (na prox. posicao vazia)
	j read	

###############################################################################################################################

close_file:
	# Antes de terinar a funcao, armazena em $s1 o numero de bytes lidos na ultima iteracao
	add $s1, $v0, $zero
	...
	# Em $s0 ja se tem o endereco base do array de bytes lido
	
	# calcula o numero de bytes lidos antes da ultima iteracao
	lw  $t1, min_size	
	add $t2, $t2, -1
	mul $t0, $t1, $t2	# multiplica o contador pelo limite max por iteracao
	add $s1, $s1, $t0	# s1 = bytes da ultima + bytes antes da ultima
	jr $ra

###############################################################################################################################

allocate_memory:
# Armazena em $v0 o endereco do primeiro byte dentre $a0 * min_size bytes alocados
	lw  $t0, min_size	# carrega numero inicial de bytes para alocacao
	mul $t0, $t0, $a0	# multiplica numero inicial pelo fator do parametro 
	add $a0, $zero, $t0	# carrega como argumento da syscall o numero de bytes a ser alocado
	li  $v0, 9		# carrega chamada de sistema para alocar memoria
	syscall			# syscall: aloca $a0 * min_size bytes e devolve em $v0 o endereco do primeiro deles
	jr $ra			

copy_byte_array:
# copia de origem ($a0) para destino ($a1), um determinado numero de bytes ($a2)
# nao retorna nada, mantendo intacto valor de $v0, $a0, $a1 e $a2
	add  $t0, $zero, $zero 	#inicializa indice do array com 0
	
copy_byte_array_loop:	
	slt  $t3, $t0, $a2	# se indice = tamanho, termina funcao
	beq  $t3, 0, back_stack
...
# converte o array de bytes cujo endereco base esta em $s0 em um array de inteiros
# ignora bytes que definam espacamento ou quebra de linha (tanto em windows como em unix)
# armazena o endereco base do array de inteiros resultantes em $s2
# o array de inteiros resutante tem o mesmo tamanho que o valor que $s1 possuir
	add  $t0, $s0, $zero   		# pega o endereco base do array de bytes 
	add  $t1, $zero, $zero   	# inicia contador de leitura
	add  $t2, $zero, $zero		# inicia contador de escrita
	add  $t3, $zero, $zero		# inicia variavel para inteiro lido
	addi $t8, $zero, 1		# inicia indicador de necessidade de teste pela
					# existência de sinal '-' no inteiro em construcao, default 1 (necessario)
	add  $t9, $zero, $zero		# inicia variavel para indicar se inteiro sera positivo ou negativo
			
	add $a0, $s1, $zero		# carrega o numero de bytes lidos do arquivo
	sll $a0, $a0, 2			# multiplica por 4 pois bytes serao convertidos para int, que ocupa 4 bytes
	li  $v0, 9			# carrega chamada de sistema para alocacao de memoria
	syscall 			# syscall: aloca $a0 bytes e retorna endereco do primeiro em $v0
	add $s2, $v0, $zero		# salva em $s2 o endereco base do array de inteiros
	
conversion_loop:
	beq  $s1, $t1, end_conversion_loop
					# se o valor do contador = numero de bytes lidos do arquivo, termina loop de conversao
	add  $t4, $t1, $t0   		# soma o contador ao endereco base, pois esta enderecando byte a byte
	lb   $t5, 0($t4)       		# armazena em $t5 o elemento do array

# $t0 = endereco base do array de bytes, $t1 = contador de leitura, $t2 = contador de escrita, 
# $t3 = var para o inteiro sendo construido, $t5 = ultimo byte lido

	addi $sp, $sp, -28		# aloca espaco na pilha para salvar variaveis
	sw   $ra, 24($sp)		# salva endereco de retorno atual
	sw   $fp, 20($sp)		# salva frame pointer atual
	sw   $t0, 16($sp)		# salva endereco base do array de bytes
	...
	add $a0, $t5, $zero		# (parametro) byte lido do array
	jal new_line_unix
	beq $v0, 1, restore_convert_to_int_array 
	# se e nova linha, continua restaurando estado (usa $v0 posteriormente)
	
	lw   $t1, 12($sp)		# restaura contador de leitura para obter segundo argumento
	lw   $t0, 16($sp)		# restaura endereco base do array para obter segundo argumento
	addi $t1, $t1, 1		# incrementa contador para obter proximo byte
	add  $t4, $t1, $t0   		# soma o contador ao endereco base, pois esta enderecando byte a byte
...
restore_convert_to_int_array:	
	lw   $ra, 24($sp)		# restaura endereco de retorno atual
	lw   $fp, 20($sp)		# restaura frame pointer atual
	lw   $t0, 16($sp)		# restaura endereco base do array de bytes
	...
	addi $sp, $sp, 24		# desaloca espaco na pilha para salvar variaveis
	...
	beq $v0, 1, conversion_loop		# se foi detectada nova linha no unix, deve ignorar 1 byte seguinte
						# pois e apenas caractere especial (Line Feed)

	addi $t1, $t1, 1			# incrementa contador, pois se detectou nova linha no windows, deve ignorar os 2 bytes seguintes
						# visto que no windows, sao 2 bytes para nova linha = (CR)(LF)
	beq  $v0, 2, conversion_loop
	
	addi $t1, $t1, -2			# senao detectou em nenhum cenario, volta contador pro estado inicial antes desses testes
					
convert_byte:	
# $t0 = endereco base do array de bytes, $t1 = contador de leitura, $t2 = contador de escrita, 
# $t3 = var para o inteiro sendo construido, $t5 = ultimo byte lido
# $s2 = endereco base do array de inteiros
	addi $t1, $t1, 1    			# incrementa contador de leitura

	li   $t7, 1				# indica que deve-se fazer um 'flush' de $t3 ao terminar o conversion_loop
	
	beqz $t8, continue_convert_byte 	# se nao deve testar, continua conversao
	bne  $t5, 45, continue_convert_byte	# se nao e igual a '-', continua conversao
	add  $t9, $zero, 1			# seta indicador de negatividade do inteiro para true
	add  $t5, $zero, 48			# zera $t5 para nao interpretar 45 como parte do numero
	add  $t8, $zero, $zero			# ja testou, nao deve testar ate o prox. int
	
continue_convert_byte:
	beq  $t5, 32, add_int	# verifica se e espaco, se for adiciona o inteiro lido ate agora
	addi $t5, $t5, -48	# senao, subtrai 48 para converter de ASCII para int
	
	mul  $t3, $t3, 10	# avanca casa decimal na variavel a ser escrita no array
...
add_int:
	...
	add $t6, $t6, $s2	# soma deslocamento (contador de escrita) ao endereco base do array de inteiro
	
	seq  $t4, $t9, 1	# testa se o inteiro deve ser negativo
	beqz $t4, do_add_int	# se nao e, pula logo para a insercao no vetor
	mul  $t3, $t3, -1
	
do_add_int:
	sw  $t3, 0($t6)		# armazena inteiro construido da posicao calculada
	li  $t7, 0		# indica que nao deve-se fazer um 'flush' de $t5 ao terminar o conversion_loop, visto que ja escreveu $t3
	
	add  $t3, $zero, $zero	# zera valor da var para inteiro sendo construido
	addi $t8, $zero, 1      # indica que deve-se testar por presenca de sinal
	add  $t9, $zero, $zero	# zera indicador de presenca de sinal
	addi $t2, $t2, 1 	# incrementa contador de escrita 
	j conversion_loop		

end_conversion_loop:
	beq $t7, 1, add_int
	add $s4, $t2, $zero	# armazena em $s4 o numero de inteiros escritos
	jr $ra  

###############################################################################################################################

new_line_windows:
# retorna 2 se conteudo em $a0 e $a1 sao bytes que indicam nova linha no windows.
...
new_line_unix:
# retorna 1 se conteudo em $a0 e um byte que indica nova linha em sistemas unix.
# retorna 0 caso contrario
	add $t0, $a0, -10
...
type_of_sort: 
	...
	bne $a1, $zero, exit_error #se o resultado de $a1 nao for zero e porque deu erro
	
	add $v0, $a0, $zero #colocando no registrador de retorno o resultado do tipo de ordenacao escolhido
...
ordena:	
	...
	j exit_error # se nao escolheu certo, sai do programa
...
quicksort:
# $a0 - endereco do primeiro byte do array de int
# $a1 - tamanho do array
	addi $a2, $a1, -1
	add  $a1, $zero, $zero
	addi $sp, $sp, -4
...
# $a2 - posicao final
	...
	add  $t0, $v0, $zero	# armazena indice da particao  		
...
# $a0 - Endereco base, $a1 - indice do primeiro elemento, $a2 - indice do ultimo elemento
# retorna posicao do divisor (indice do pivot que divide o array em particoes)
...
# $t2 - indice para percorrer sub-array
	...
	addi $t1, $a1, -1	# inicializa variavel do limite de menores ou iguais
	add  $t2, $a1, $zero	# inicializa indice com primeira posicao do sub array
	
particao_loop:
	slt  $t3, $t2, $a2	# enquanto indice < ultimo ind.
	...
	sle  $t5, $t4, $t0	# testa se e menor ou igual ao pivot
	beq  $t5, $zero, continue_particao_loop # se e maior, continua loop
	addi $t1, $t1, 1	# senao, incrementa limite de menores ou iguais, pois achou um novo menor ou igual

# faz swap
	sll  $t5, $t1, 2
	add  $t5, $t5, $a0	# calcula endereco em memoria do limite de menores ou iguais
...
loop_insertion:
	...
	beq $t4, $zero, back_stack # senao sai do loop
	
	sll $t4, $t0, 2 # faz i * 4  para calcular posicao do array
	add $t4, $t4, $a0 #calcula posicao do array
	lw $t2, 0($t4) # salva em v o valor de A[i]
	add $t1, $zero, $t0 # deixa j igual a i
	
	addi $sp, $sp, -4 # aloca posicao na pilha
	sw   $ra, 0($sp) # salva a posicao de $ra na pilha
	...
	sll $t4, $t1, 2 # faz j * 4 para calcular a posicao do array
	add $t4, $t4, $a0 #pega posicao do A[j]
	sw $t2, 0($t4) # A[j] = v
...
loop_to_move_insertion:
	...
	addi $t4, $t1, -1 # $t4 = j - 1
	sll $t4, $t4, 2 # faz (j-1) * 4 para calcular a posicao do array
	add $t5, $t4, $a0 #calcula posicao do array
	lw $t5, 0($t5) # pega A[j-1]
	slt $t4, $t2, $t5 # verifica se v < A[j-1]
	beq $t4, $zero, back_stack # se nao for, termina o loop
	
	sll $t4, $t1, 2  # faz j * 4 para calcular a posicao do array
	add $t4, $t4, $a0 #calcula posicao de A[j]
...
convert_to_byte_array:
	...
	add $t2, $v0, $zero	# inicializa variavel para armazenar a string a ser escrita
	add $s0, $v0, $zero
	
convert_to_byte_array_loop:
	beq $t0, $s4, end_convert_to_byte_array	# se e do tamanho do array de inteiros, termina
	sll $t3, $t0, 2		# multiplica por 4, enderecamento por word
	add $t3, $t3, $s2	# calcula o endereco do elemento atual do array
...
get_numbers:
	...
	slt $t6, $t3, $t4 #verifica se o numero e menor que a potencia
	beq $t6, $zero, write_quotient #se nao for menor, imprime o quociente
...
write_quotient:
	div $t3, $t4 #divide o numero pela potencia e coloca em lo o quociente e em hi o resto
	mflo $t6 #quociente
    mfhi $t3 # atualiza o numero para o resto entre o numero antigo e a potencia
...
calc_pot:
	div $t3, $t4 #divide o numero pela potencia e coloca em lo o quociente
	mflo $t6
	slt $t6, $t6, $t5 #verifica se o numero / potencia e menor que 10
	beq $t6, 1, back_stack
...
write_file:
	...
	addi $t1, $zero, 13 	# Comeca quebra de linha x2
...
file_name: 		# nome do arquivo que sera colocado pelo usuario
	.asciiz ""	
array:
	.align 2
	.space 1024
min_size:
	.word 256		# numero inicial de bytes a serem lidos do arquivo
